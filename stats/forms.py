...
class StatsOptions(forms.Form):
    ...
    **Validation**
    ...
    group_by_choices = [
        ("pilot__callsign", "Pilot"),
        ("aircraft", "Aircraft"),
        ("mission__name", "Mission"),
    ]
    group_by = forms.MultipleChoiceField(
        label="Group By", choices=group_by_choices, initial="Pilot"
    )
    ...
    aircraft_filter = forms.ModelChoiceField(
        queryset=Aircraft.objects.all(), required=False
    )
...
class MisForm(ModelForm):
    aircraft = forms.ModelChoiceField(
        queryset=Aircraft.objects.all().order_by("aircraft"), required=True
    )
