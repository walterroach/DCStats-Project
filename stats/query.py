...
def execute(options):
    ...
    **arg**
    ...
    missions = Stats.objects.filter(
        mission__date__range=(options["start_date"], options["end_date"])
    )
...
def new_stats(options):
    ...
    **args**
    ...
    stats = (
        qset.filter(
            new__gte=options["unlogged_only"],
            mission__date__range=(options["start_date"], options["end_date"]),
        )
        .annotate(
            in_air_hours=Sum("in_air_sec") / 3600,
